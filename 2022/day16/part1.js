/**
 * Solutions for Advent of Code 2022, Day 16, Part 1.
...
let queue = [{
  path: ['AA'],
  minutesToEruption: 30,
  totalPressureReleased: 0
}];
let finishedPaths = [];

while (queue.length != 0) {
  let pathInfo = queue.pop();
...
console.dir({
  ...
  // finishedPaths
  maxPressureReleased: finishedPaths.map(pathInfo => pathInfo.totalPressureReleased).reduce((max, cur) => Math.max(max, cur))
}, {depth: null});