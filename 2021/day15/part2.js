/**
 * Solutions for Advent of Code 2021, Day 15, Part 2.
...
const ORIGINAL_HEIGHT = riskLevelMatrix.length;
const ORIGINAL_WIDTH = riskLevelMatrix[0].length;

// Expand our matrix.
for (let i = 0; i < ORIGINAL_HEIGHT; i++) {
  for (let j = ORIGINAL_WIDTH; j < ORIGINAL_WIDTH * 5; j++) {
    let prev = riskLevelMatrix[i][j - ORIGINAL_WIDTH];
    riskLevelMatrix[i][j] = (prev + 1) > 9 ? 1 : prev + 1;
  }
}
for (let i = ORIGINAL_HEIGHT; i < ORIGINAL_HEIGHT * 5; i++) {
  riskLevelMatrix[i] = new Array(ORIGINAL_WIDTH * 5);

  for (let j = 0; j < ORIGINAL_WIDTH * 5; j++) {
    let prev = riskLevelMatrix[i - ORIGINAL_HEIGHT][j];
    riskLevelMatrix[i][j] = (prev + 1) > 9 ? 1 : prev + 1;
  }
}
...
// Implement a shitty form of https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm
// without using a min heap.
