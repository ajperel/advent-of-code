/**
 * Solutions for Advent of Code 2021, Day 11, Part 2.
...
let checkAllOctopusesFlashed = (grid) => grid.every(row => row.every(cell => cell === 0));
let step = 0;

while (!checkAllOctopusesFlashed(grid)) {
  step += 1;
  raiseEnergyLevel(grid);
  processOctopusFlashes(grid);
}
