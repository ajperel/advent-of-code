/**
 * Solutions for Advent of Code 2021, Day 23, Part 2.
...
let initialPositions = [
  ...
  { type: match2[1], position: [2, 4] },
  { type: match2[2], position: [4, 4] },
  { type: match2[3], position: [6, 4] },
  { type: match2[4], position: [8, 4] },
  { type: 'A', position: [8, 2]},
  { type: 'A', position: [6, 3]},
  { type: 'B', position: [6, 2]},
  { type: 'B', position: [4, 3]},
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    if (position[0] === destinationX[type] &&
        constantPositions.every(({type: t1, position: p1}) =>
            p1[0] != position[0] ||
            p1[1] < position[1] ||
            (t1 === type && p1[1] > position[1]))) {
      continue;
    }

    // If we're in a room but blocked by someone else we have no possible moves.
    if (position[1] > 1 && constantPositions.some(({position: p1}) =>
        position[0] == p1[0] && position[1] > p1[1])) {
      continue;
    }
