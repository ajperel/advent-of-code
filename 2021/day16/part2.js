/**
 * Solutions for Advent of Code 2021, Day 16, Part 2.
...
performPacketOps = (packet) => {
  let result;

  let subResults = packet.subPackets.map(p => performPacketOps(p));

  switch (packet.id) {
    case 0:
      result = subResults.reduce((sum, n) => sum + n);
      break;
    case 1:
      result = subResults.reduce((prduct, n) => prduct * n);
      break;
    case 2:
      result = Math.min(...subResults);
      break;
    case 3:
      result = Math.max(...subResults);
      break;
    case 4:
      result = packet.value;
      break;
    case 5:
      result = subResults[0] > subResults[1] ? 1 : 0;
      break;
    case 6:
      result = subResults[0] < subResults[1] ? 1 : 0;
      break;
...
let answer = packets.map(p => performPacketOps(p));
